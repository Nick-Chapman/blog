...
Quarter primitives are accessed via a flat dispatch table, indexed by an Ascii charachter. This allows a maximum of 128 primitives; although it is probably wise to restrict ourselves to just the printable characters 33 (`!`) to 126 (`~`).
...
The code quad `^*?>` is executed similarly, but for the muliplication primitive.
...
This defines comment suppot both in Quarter, via the definition `:(`, but it also constructs an entry in the Forth dictionary with ``H@^(`0`E`` so comments will be available in Forth once we enter the bootstrap interpreter beig defined. Moving swiftly on...
...
This interpreter does not handle numeric literals, but only words which are looked up in the Forth dictionary. The core functionality for reading a blank-delimited word and dictionary lookup is defined in sub-definitions for `w` and `f`. But we can see the overall instructure of the interpreter here.
